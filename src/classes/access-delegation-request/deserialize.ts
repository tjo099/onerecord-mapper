...
import type { AccessDelegationRequest } from './schema.js'
import { AccessDelegationRequestSchema } from './schema.js'

export function deserializeAccessDelegationRequest(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<AccessDelegationRequest> {
  // 1. Pre-Zod sanity
  ...
  const r = AccessDelegationRequestSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  // 5. Null-prototype clone
  return { ok: true, value: nullProtoClone(r.data) as AccessDelegationRequest }
}
