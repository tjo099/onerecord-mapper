...
import type { AccessDelegation } from './schema.js'
import { AccessDelegationSchema } from './schema.js'

export function deserializeAccessDelegation(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<AccessDelegation> {
  // 1. Pre-Zod sanity
  ...
  const r = AccessDelegationSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as AccessDelegation }
}
