...
import type { AccountNumber } from './schema.js'
import { AccountNumberSchema } from './schema.js'

export function deserializeAccountNumber(
  input: unknown,
  ...
  const r = AccountNumberSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as AccountNumber }
}
