...
// src/classes/booking-request/deserialize.ts
import type { BookingRequest } from './schema.js'
import { BookingRequestSchema } from './schema.js'

export function deserializeBookingRequest(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<BookingRequest> {
  ...
  const r = BookingRequestSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as BookingRequest }
}
