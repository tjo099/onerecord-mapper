...
// src/classes/booking-segment/deserialize.ts
import type { BookingSegment } from './schema.js'
import { BookingSegmentSchema } from './schema.js'

export function deserializeBookingSegment(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<BookingSegment> {
  ...
  const r = BookingSegmentSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as BookingSegment }
}
