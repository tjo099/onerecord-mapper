...
import type { ChangeRequest } from './schema.js'
import { ChangeRequestSchema } from './schema.js'

export function deserializeChangeRequest(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<ChangeRequest> {
  ...
  const r = ChangeRequestSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as ChangeRequest }
}
