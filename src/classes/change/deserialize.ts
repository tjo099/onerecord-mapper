...
export function deserializeChange(input: unknown, opts: DeserializeOpts = {}): ParseResult<Change> {
  ...
  const r = ChangeSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as Change }
}
