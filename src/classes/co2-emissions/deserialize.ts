...
import type { CO2Emissions } from './schema.js'
import { CO2EmissionsSchema } from './schema.js'

export function deserializeCO2Emissions(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<CO2Emissions> {
  ...
  const r = CO2EmissionsSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as CO2Emissions }
}
