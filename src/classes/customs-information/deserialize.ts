...
import type { CustomsInformation } from './schema.js'
import { CustomsInformationSchema } from './schema.js'

export function deserializeCustomsInformation(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<CustomsInformation> {
  ...
  const r = CustomsInformationSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as CustomsInformation }
}
