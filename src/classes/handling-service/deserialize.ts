...
import type { HandlingService } from './schema.js'
import { HandlingServiceSchema } from './schema.js'

export function deserializeHandlingService(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<HandlingService> {
  ...
  const r = HandlingServiceSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as HandlingService }
}
