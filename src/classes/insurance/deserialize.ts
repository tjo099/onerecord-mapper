...
import type { Insurance } from './schema.js'
import { InsuranceSchema } from './schema.js'

export function deserializeInsurance(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<Insurance> {
  ...
  const r = InsuranceSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as Insurance }
}
