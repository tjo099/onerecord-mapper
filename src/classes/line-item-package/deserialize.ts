...
import type { LineItemPackage } from './schema.js'
import { LineItemPackageSchema } from './schema.js'

export function deserializeLineItemPackage(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<LineItemPackage> {
  ...
  const r = LineItemPackageSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as LineItemPackage }
}
