...
import type { LogisticsEvent } from './schema.js'
import { LogisticsEventSchema } from './schema.js'

export function deserializeLogisticsEvent(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<LogisticsEvent> {
  ...
  const r = LogisticsEventSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as LogisticsEvent }
}
