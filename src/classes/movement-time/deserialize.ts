...
import type { MovementTime } from './schema.js'
import { MovementTimeSchema } from './schema.js'

export function deserializeMovementTime(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<MovementTime> {
  ...
  const r = MovementTimeSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as MovementTime }
}
