...
import type { OtherCharge } from './schema.js'
import { OtherChargeSchema } from './schema.js'

export function deserializeOtherCharge(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<OtherCharge> {
  ...
  const r = OtherChargeSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as OtherCharge }
}
