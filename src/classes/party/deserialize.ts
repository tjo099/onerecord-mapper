...
import type { Party } from './schema.js'
import { PartySchema } from './schema.js'

export function deserializeParty(input: unknown, opts: DeserializeOpts = {}): ParseResult<Party> {
  ...
  const r = PartySchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as Party }
}
