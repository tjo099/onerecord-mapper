...
export function deserializePerson(input: unknown, opts: DeserializeOpts = {}): ParseResult<Person> {
  // 1. Pre-Zod sanity
  ...
  const r = PersonSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as Person }
}
