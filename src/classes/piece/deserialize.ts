...
import type { Piece } from './schema.js'
import { PieceSchema } from './schema.js'

export function deserializePiece(input: unknown, opts: DeserializeOpts = {}): ParseResult<Piece> {
  // 1. Pre-Zod sanity
  ...
  // 3. Empty-array rejection BEFORE Zod (v3 plan correction)
  ...
  const r = PieceSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  // 5. Null-prototype clone output
  return { ok: true, value: nullProtoClone(r.data) as Piece }
}
