...
import type { RegulatedEntity } from './schema.js'
import { RegulatedEntitySchema } from './schema.js'

export function deserializeRegulatedEntity(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<RegulatedEntity> {
  ...
  const r = RegulatedEntitySchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as RegulatedEntity }
}
