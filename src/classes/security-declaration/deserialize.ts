...
import type { SecurityDeclaration } from './schema.js'
import { SecurityDeclarationSchema } from './schema.js'

export function deserializeSecurityDeclaration(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<SecurityDeclaration> {
  ...
  const r = SecurityDeclarationSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as SecurityDeclaration }
}
