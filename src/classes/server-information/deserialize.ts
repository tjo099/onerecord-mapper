...
import type { ServerInformation } from './schema.js'
import { ServerInformationSchema } from './schema.js'

export function deserializeServerInformation(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<ServerInformation> {
  // 1. Pre-Zod sanity
  ...
  const r = ServerInformationSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as ServerInformation }
}
