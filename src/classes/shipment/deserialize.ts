...
import type { Shipment } from './schema.js'
import { ShipmentSchema } from './schema.js'

export function deserializeShipment(
  input: unknown,
  ...
  const r = ShipmentSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as Shipment }
}
