...
import type { SubscriptionRequest } from './schema.js'
import { SubscriptionRequestSchema } from './schema.js'

export function deserializeSubscriptionRequest(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<SubscriptionRequest> {
  ...
  const r = SubscriptionRequestSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as SubscriptionRequest }
}
