...
import type { TransportMovement } from './schema.js'
import { TransportMovementSchema } from './schema.js'

export function deserializeTransportMovement(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<TransportMovement> {
  ...
  const r = TransportMovementSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as TransportMovement }
}
