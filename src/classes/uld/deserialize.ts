...
import type { ULD } from './schema.js'
import { ULDSchema } from './schema.js'

export function deserializeULD(input: unknown, opts: DeserializeOpts = {}): ParseResult<ULD> {
  ...
  const r = ULDSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as ULD }
}
