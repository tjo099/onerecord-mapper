...
import type { VerificationRequest } from './schema.js'
import { VerificationRequestSchema } from './schema.js'

export function deserializeVerificationRequest(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<VerificationRequest> {
  // 1. Pre-Zod sanity
  ...
  const r = VerificationRequestSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as VerificationRequest }
}
