...
import type { UnverifiedVerification } from './schema.js'
import { VerificationSchema } from './schema.js'

export function deserializeVerification(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<UnverifiedVerification> {
  // 1. Pre-Zod sanity
  ...
  // 3. Empty-array rejection (v3 plan correction)
  ...
  // 4. Zod parse, lift safeIri-tagged issues
  const r = VerificationSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  // 5. Null-prototype clone, brand as UnverifiedVerification
  return { ok: true, value: nullProtoClone(r.data) as UnverifiedVerification }
}
