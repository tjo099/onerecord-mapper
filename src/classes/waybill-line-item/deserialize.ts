...
import type { WaybillLineItem } from './schema.js'
import { WaybillLineItemSchema } from './schema.js'

export function deserializeWaybillLineItem(
  input: unknown,
  opts: DeserializeOpts = {},
): ParseResult<WaybillLineItem> {
  ...
  const r = WaybillLineItemSchema.safeParse(pre.value)
  if (!r.success) {
  ...
  return { ok: true, value: nullProtoClone(r.data) as WaybillLineItem }
}
